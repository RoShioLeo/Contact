...
import static cloud.lemonslice.contact.common.container.MenuTypeRegistry.WRAPPING_PAPER_CONTAINER;
        ...
        for (int i = 0; i < 4; i++)
        {
            addSlot(new SlotItemHandler(inputs, i, 35 + 20 * i, 16)
            {
                ...
                public boolean mayPlace(@Nonnull ItemStack stack)
                {
                    ...
                    return id != null && (!CommonConfig.Mail.blacklistID.get().contains(id.toString()) || !CommonConfig.Mail.blacklistID.get().contains(id.getPath()));
                }
            });
        }
        for (int i = 0; i < 3; ++i)
