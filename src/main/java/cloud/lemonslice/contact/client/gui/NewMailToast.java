...
public class NewMailToast implements IToast
{
    ...
    public Visibility func_230444_a_(MatrixStack matrixStack, ToastGui gui, long ticks)
    {
        ...
        List<IReorderingProcessor> list = gui.getMinecraft().fontRenderer.trimStringToWidth(new TranslationTextComponent("info.contact.new_mail.desc"), 125);
        int i = 16776960;
        if (list.size() == 1)
        {
            gui.getMinecraft().fontRenderer.drawText(matrixStack, new TranslationTextComponent("info.contact.new_mail.title"), 30.0F, 7.0F, i | -16777216);
        ...
        else
        {
            if (ticks < 1500L)
            {
                ...
                gui.getMinecraft().fontRenderer.drawText(matrixStack, new TranslationTextComponent("info.contact.new_mail.desc"), 30.0F, 11.0F, i | k);
            }
            else
            {
