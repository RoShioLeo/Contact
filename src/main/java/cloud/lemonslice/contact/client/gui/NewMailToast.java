...
public class NewMailToast implements Toast
{
    ...
    public Visibility render(PoseStack matrixStack, ToastComponent gui, long ticks)
    {
        ...
        List<FormattedCharSequence> list = gui.getMinecraft().font.split(new TranslatableComponent("info.contact.new_mail.desc"), 125);
        int i = 16776960;
        if (list.size() == 1)
        {
            gui.getMinecraft().font.draw(matrixStack, new TranslatableComponent("info.contact.new_mail.title"), 30.0F, 7.0F, i | -16777216);
        ...
        else
        {
            if (ticks < 1500L)
            {
                ...
                gui.getMinecraft().font.draw(matrixStack, new TranslatableComponent("info.contact.new_mail.desc"), 30.0F, 11.0F, i | k);
            }
            else
            {
