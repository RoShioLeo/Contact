...
public class PostboxGui extends AbstractContainerScreen<PostboxContainer>
{
    ...
    protected void init()
    {
        ...
        this.buttonSend = this.addWidget(new IconButton(offsetX + 97, offsetY + 26, 10, 9, new TranslatableComponent("tooltip.contact.postbox.send"), button -> send(), this::buttonTooltip));

        this.minecraft.keyboardHandler.setSendRepeatsToGui(true);
        this.nameField = this.addRenderableWidget(new EditBox(this.font, offsetX + 42, offsetY + 26, 43, 9, new TranslatableComponent("info.contact.postbox.addressee")));
    ...
    private void buttonTooltip(Button button, PoseStack poseStack, int mouseX, int mouseY)
    {
        if (button.isHoveredOrFocused())
        {
            ...
            {
                GuiHelper.drawTooltip(this, poseStack, mouseX, mouseY, button.x, button.y, button.getWidth(), button.getHeight(), Lists.newArrayList(new TranslatableComponent("tooltip.contact.postbox.enquire")));
            }
        }
    }
    ...
    {
        this.font.draw(poseStack, new TranslatableComponent("info.contact.postbox.addressee"), 40, 14, 0xE6E6E6);
        switch (menu.status)
        {
            case 0:
            {
            ...
            case 2:
            {
                ...
                if (width > 38)
                {
                    ...
                    if (menu.time == 0)
                    {
                        this.font.draw(poseStack, new TranslatableComponent("info.contact.postbox.instant"), 141 - width / 2, 22, 0x1A1A1A);
                    }
                    else
                    {
                        this.font.draw(poseStack, new TranslatableComponent("info.contact.postbox.time", min, sec), 141 - width / 2, 22, 0x1A1A1A);
                    }
                }
                else
                {
                    ...
                    if (menu.time == 0)
                    {
                        this.font.draw(poseStack, new TranslatableComponent("info.contact.postbox.instant"), 122, 22, 0x1A1A1A);
                    }
                    else
                    {
                        this.font.draw(poseStack, new TranslatableComponent("info.contact.postbox.time", min, sec), 122, 22, 0x1A1A1A);
                    }
                }
                return;
            }
